...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Renders the main application UI after login
    function renderApp(user) {
        ...
        appContainer.innerHTML = `
            ...
            <main id="main-content" class="container mx-auto p-4 space-y-6">
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            if (docSnap.exists() && docSnap.data().settings) {
                ...
                // Firestoreに設定があれば、storeの値を上書き
                if (typeof settings.isSalaryReportVisible === 'boolean') {
                    store.isSalaryReportVisible = settings.isSalaryReportVisible;
                }
            }
            renderApp(user);
        } else {
